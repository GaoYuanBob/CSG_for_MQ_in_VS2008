...
public:
	Mesh_CSG() {};
	Mesh_CSG(Mesh_CSG &src);
	Mesh_CSG(const RawMesh<VertData, TriData> &raw);
	virtual ~Mesh_CSG() {};

	void operator=(Mesh_CSG &src);
	...
	void disjointUnion(const Mesh_CSG &cp);
	...
	Isct pick(Ray3d_CSG ray);

	bool isClosed(); // checks if the Mesh_CSG is closed
...
public: // ISCT (intersections) module
	...
	bool isSelfIntersecting(); // is the Mesh_CSG self-intersecting?
							   // TESTING
...
public: // BOOLean operation module
		// all of the form
		//      this = this OP rhs
	void boolUnion(Mesh_CSG &rhs);
	void boolDiff(Mesh_CSG &rhs);
	void boolIsct(Mesh_CSG &rhs);
	void boolXor(Mesh_CSG &rhs);
